...
USE_PROCD=1
PROG=/usr/bin/ssh-tunnel

start_service() {
    procd_open_instance [instance_name]
    procd_set_param command "$PROG"

    # we don't use a custom ssh-tunnel logdir
    # instead, redirect stdout/stderr to default OpenWRT logd
    # use `logread -e ssh-tunnel` to view logs
    procd_set_param stdout 1
    procd_set_param stderr 1

    # respawn automatically if something died, be careful if you have an alternative process supervisor
    # we wait 30 seconds before restarting the ssh-tunnel
